...
class ActionCompiler:
    ...
    def compile_decision(
        ...
        *,
        ...
        if plan is None:
            plan = _deterministic_plan_from_message(
                message,
                context=context,
                latest_observation=latest_observation,
            )
...
def _deterministic_plan_from_message(
    message: str,
    *,
    context: dict[str, Any] | None,
    latest_observation: dict[str, Any] | None,
) -> ClientActionPlan | None:
    """Conservative offline compiler for common local-client action requests.

    This fallback intentionally covers only high-signal action phrasing so an
    unavailable intent model does not turn ordinary chat into side effects.
    """
    ...
    actions: list[ClientActionV2] = []

    shell_command = _extract_shell_command(text)
    if shell_command:
        return ClientActionPlan(
            mode="direct",
            goal="run terminal command",
            confidence=0.78,
            reason="deterministic terminal action fallback",
            actions=[
                ClientActionV2(
                    name="terminal.run",
                    payload=shell_command,
                    args={"command": shell_command},
                    description=f"Run terminal command: {shell_command}",
                    requires_confirm=True,
                )
            ],
        )

    if _looks_like_screen_observation(normalized):
        return ClientActionPlan(
            mode="direct",
            goal="observe screen",
            confidence=0.82,
            reason="deterministic screen observation fallback",
            actions=[
                ClientActionV2(
                    name="screen.screenshot",
                    target="full_screen",
                    args={"purpose": "observe_screen"},
                    description="Capture the current screen for observation",
                    requires_confirm=False,
                )
            ],
        )

    result_index = _requested_result_index(text)
    if result_index is not None and _looks_like_search_result_request(normalized):
        return ClientActionPlan(
            mode="direct",
            goal="open search result",
            confidence=0.82,
            reason="deterministic search-result fallback",
            actions=[
                ClientActionV2(
                    name="browser.select_result",
                    target="active_tab",
                    args={"index": result_index},
                    description=f"Open search result {result_index}",
                    requires_confirm=False,
                )
            ],
        )

    search_query = _extract_browser_search_query(text)
    if search_query:
        browser = _browser_from_message(text) or _context_string(context, "default_browser")
        return ClientActionPlan(
            mode="direct",
            goal="search browser",
            confidence=0.8,
            reason="deterministic browser search fallback",
            actions=[
                ClientActionV2(
                    name="browser.search",
                    args=_without_none_v2(
                        {
                            "query": search_query,
                            "browser": browser,
                            "search_engine": _context_string(context, "search_engine"),
                        }
                    ),
                    description=f"Search browser for {search_query}",
                    requires_confirm=False,
                )
            ],
        )
    ...
    typed_text = _extract_typed_text(text)
    if app_target and typed_text:
        actions = [
            ClientActionV2(
                name="app.open",
                target=app_target,
            ...
            ClientActionV2(
                ...
                description=f"Type text into {app_target}",
                requires_confirm=False,
            ),
        ]
        return ClientActionPlan(
            mode="direct_sequence",
            goal=f"open {app_target} and type text",
            confidence=0.78,
            reason="deterministic app-open/type fallback",
            actions=actions,
        )

    browser_target = _browser_from_message(text) or _context_string(context, "default_browser")
    if (
        browser_target
        and _looks_like_open_browser_request(normalized)
        and not any(token in normalized for token in ("search", "검색", "찾아"))
    ):
        return ClientActionPlan(
            mode="direct",
            goal="open browser",
            confidence=0.76,
            reason="deterministic browser-open fallback",
            actions=[
                ClientActionV2(
                    name="browser.open",
                    args={"browser": browser_target},
                    description=f"Open {browser_target}",
                    requires_confirm=False,
                )
            ],
        )

    # If the user asks to use the latest observation/result but no concrete
    # action is identifiable, leave the turn to the normal conversational path.
    _ = latest_observation
    return None
...
def _looks_like_screen_observation(normalized: str) -> bool:
    return any(
...
def _looks_like_search_result_request(normalized: str) -> bool:
    return any(
        phrase in normalized
        for phrase in (
            "search result",
...
def _looks_like_open_browser_request(normalized: str) -> bool:
    return any(
        phrase in normalized
        for phrase in (
            "open browser",
        ...
        phrase in normalized
        for phrase in ("open", "launch", "켜", "열", "실행")
    )
...
def _requested_result_index(text: str) -> int | None:
    ...
    for word, index in ordinal_words.items():
        if word in normalized:
            return index
    match = re.search(r"(\d+)\s*(?:번째|번|st|nd|rd|th)?", normalized)
    if match:
        return int(match.group(1))
    return None


def _browser_from_message(text: str) -> str | None:
    normalized = text.lower()
    if "chrome" in normalized or "크롬" in normalized:
        return "chrome"
    if "safari" in normalized or "사파리" in normalized:
        return "safari"
    if "edge" in normalized or "엣지" in normalized:
        return "edge"
    if "firefox" in normalized or "파이어폭스" in normalized:
        return "firefox"
    return None


def _extract_browser_search_query(text: str) -> str | None:
    normalized = text.lower()
    if not any(token in normalized for token in ("search", "검색", "찾아", "찾아봐")):
        return None
    patterns = [
        r"(?:search(?:\s+for)?|look\s+up)\s+(.+?)(?:\s+in\s+(?:the\s+)?browser|\s+on\s+\w+)?$",
        r"(?:브라우저|크롬|사파리|엣지|파이어폭스)?(?:에서)?\s*(.+?)\s*(?:검색|찾아봐|찾아줘)(?:해)?(?:줘|주세요)?(?:\s|$)",
        r"(.+?)\s*(?:검색|찾아봐|찾아줘)(?:해)?(?:줘|주세요)?(?:\s|$)",
    ]
    for pattern in patterns:
        match = re.search(pattern, text, re.IGNORECASE)
        if not match:
            continue
        query = _clean_extracted_phrase(match.group(1))
        if query:
            return query
    return None
...
def _extract_app_target(text: str) -> str | None:
    normalized = text.lower()
    if "sublime" in normalized or "서브라임" in normalized:
        return "Sublime Text"
    app_match = re.search(
        r"(?:open|launch)\s+(.+?)\s+(?:and\s+)?(?:type|enter|write)",
        text,
        re.IGNORECASE,
    )
    if app_match:
        return _clean_extracted_phrase(app_match.group(1))
    korean_match = re.search(r"(.+?)\s*(?:켜서|열어서|실행해서).*(?:입력|타이핑)", text)
    if korean_match:
        candidate = _clean_extracted_phrase(korean_match.group(1))
        if candidate:
            return candidate
    return None


def _extract_typed_text(text: str) -> str | None:
    patterns = [
        r"(?:type|enter|write)\s+(.+?)(?:\s+into|\s+in|\s*$)",
        r"(.+?)\s*(?:입력|타이핑|작성)해?(?:줘|주세요)?$",
    ]
    for pattern in patterns:
        match = re.search(pattern, text, re.IGNORECASE)
        if not match:
            continue
        value = _clean_extracted_phrase(match.group(1))
        if value:
            if "켜서" in value or "열어서" in value or "실행해서" in value:
                value = re.split(r"켜서|열어서|실행해서", value, maxsplit=1)[-1].strip()
            return _strip_action_suffix(value)
    return None
...
def _extract_shell_command(text: str) -> str | None:
    ...
    patterns = [
        r"(?:run|execute)\s+(.+?)(?:\s+in\s+(?:terminal|shell)|$)",
        r"(?:terminal|shell|command)(?:에서|로|에)?\s+(.+?)(?:\s*(?:실행|돌려|입력)해?(?:줘|주세요)?$|$)",
        r"(?:터미널|쉘|명령)(?:에서|로|에)?\s+(.+?)(?:\s*(?:실행|돌려|입력)해?(?:줘|주세요)?$|$)",
    ]
    for pattern in patterns:
        match = re.search(pattern, text, re.IGNORECASE)
        if not match:
            continue
        command = _clean_extracted_phrase(match.group(1))
        if command:
            return _strip_action_suffix(command)
    return None
...
def _clean_extracted_phrase(value: str | None) -> str | None:
    if not isinstance(value, str):
        return None
    cleaned = value.strip(" \t\r\n\"'`“”")
    cleaned = re.sub(r"^(?:에서|으로|로)\s*", "", cleaned)
    cleaned = re.sub(r"\s*(?:해줘|해주세요|해|please)$", "", cleaned, flags=re.IGNORECASE)
    return cleaned or None


def _strip_action_suffix(value: str) -> str:
    return re.sub(
        r"\s*(?:입력|타이핑|작성|검색|찾아봐|찾아줘|실행|돌려)해?(?:줘|주세요)?$",
        "",
        value,
        flags=re.IGNORECASE,
    ).strip()
