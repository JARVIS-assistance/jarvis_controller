...
4. direct/direct_sequence 액션이면
   Controller가 core realtime 답변을 중단하고 `잠시만요!`, `요청하신 작업 시작할게요` assistant_delta를 emit한 뒤 action_dispatcher.enqueue(...)
...
Intent 결과가 `direct` 또는 `direct_sequence`로 확정되면 Controller는 core의 일반 realtime 답변을 중단하고 action dispatch 전에 별도 `assistant_delta`로 `잠시만요!`, `요청하신 작업 시작할게요`를 emit한다. Frontend는 이 문구 역시 실행 권한으로 해석하지 않고 UI 상태 문구로만 표시한다. 실제 실행 기준은 계속 `action_dispatch.action_id`와 pending queue envelope다.
