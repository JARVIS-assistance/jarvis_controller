...
2. Controller
   sLLM action intent classifier 실행

3. direct/direct_sequence 액션이면
   action_dispatcher.enqueue(...)

4. Controller -> Frontend SSE
   event: action_intent
...
X-Client-Capabilities: open_url,browser_control,app_control,keyboard_type,hotkey,terminal,calendar_control
```
...
direct action에서는 모델 답변 생성을 생략하고 짧은 완료 메시지만 보낸다.
...
| 브라우저 페이지 조작 | `browser_control` | `scroll`, `back`, `forward`, `reload`, `extract_dom`, `click_element`, `type_element` | "두번째 결과 들어가줘", "스크롤 내려줘" |
...
| 키보드/단축키 | `keyboard_type`, `hotkey` | typing, hotkey | "현재 창에 안녕하세요 입력해줘", "주소창 선택해줘" |
...
type ActionHandler = (envelope: ClientActionEnvelope) => Promise<ClientActionResult>;
```

Dispatcher 예시:

```ts
async function executeAction(envelope: ClientActionEnvelope): Promise<ClientActionResult> {
